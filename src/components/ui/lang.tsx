...
export default function Lang() {
  ...
  const [open, setOpen] = useState(false)
    ...
    {
        open ? <ul className='max-w-[108px] absolute top-12  bg-neutral-50 bg-white dark:bg-[#00132D] dark:text-[#FFFFFF] py-2 sm:py-3 rounded-xl w-full left-0"'>
            {
                            LangArr.map((e,i) => (
                                <Link key={i} href={redirectToLocale(e?.value, currentPathname)}>
