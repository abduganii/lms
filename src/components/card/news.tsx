...
export default function NewsCard({navlink='/', image,title,date,disc,className}:any) {
    return (
    <Link href={navlink} className={`group w-full ${className && className}`} >
        ...
        {disc &&   <p className="text-sm font-normal leading-6 text-left flex items-center text-[#5B6871]" dangerouslySetInnerHTML={{__html:disc}}/>}
